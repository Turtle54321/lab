...
//	BackUserActionLogService backUserActionLogService;
    ...
    public Object aroundMethod(ProceedingJoinPoint pjd, RetFormat pv) throws Throwable {
        ...
        if(pv.isPage()){
            ...
            /*HttpServletResponse response = (HttpServletResponse)HttpUtil.getResponse();
            PrintWriter out = response.getWriter();
            out.print(JsonUtil.getJsonFromObject(map));
            out.flush();
            out.close();*/
            HttpServletRequest request = HttpUtil.getHttpRequest();
            request.setAttribute("value",JsonUtil.getJsonFromObject(map));
            //暂时关闭requestId验证
            //recordActionLog(pv, map);
            return "page/inteface";
        }
